...
class Template {
	...
	public function getIncludePath( $tempPath = null ) {
        ...
        $this->templateDir = APP_PATH.APP_NAME.'/'.$this->configs['module'].'/template/'.$this->configs['template'].'/';
        $this->compileDir = APP_RUNTIME_PATH.'views/'.APP_NAME.'/'.$this->configs['module'].'/';
        $tempDir = APP_PATH.$appName.'/'.$moduleInfo[0].'/template/'.$this->configs['template'].'/';
	...
	public function importResource( $section, $type, $path ) {
        ...
        switch ( $section ) {
            ...
            case 'res' :
                $resUrl .= 'app/'.APP_NAME.'/'.$this->configs['module'].'/'.$this->configs['template'].'/';
                break;
        ...
        if ( strpos($path, '@') !== false ) {
            $path = explode('@', $path);
            $resUrl = str_replace($this->configs['module'], $path[0], $resUrl);
            $path = $path[1];
        }
