...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
	        ...
	        actionUrl=actionUrl + (actionUrl.indexOf("?") == -1 ? "?" : "&") + "editorid="+editor.id;//初始form提交地址;

	        //提交用户需要传入的参数
	        if ( editor.options.userData ) {
		        for ( var name in editor.options.userData ) {
			        actionUrl += '&' + name + '=' + editor.options.userData[name];
		        }
	        }
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.width = 113;
                    img.setAttribute('src', urlPrefix + list[i].thumb);
                    //img.setAttribute('src', urlPrefix + list[i].thumb + (list[i].thumb.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
